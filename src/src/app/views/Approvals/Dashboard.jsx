...
const Dashboard = () => {
  ...
  const handleApprovalSubmit1 = async () => {
    ...
    try {
      ...
      if (statusID === 2) {
        ...
        if (response?.status === 200) {
          const result = response?.data?.data?.listLeaveApprovalEntity?.[0]?.result;
      ...
      // Proceed to email sending if the approval/rejection was successful
      const approvalEntity =
        response.data.data.listLeaveApprovalEntity || response.data.data.listLeaveApprovalEntity;

      const { fromEmailID, toEmailID, ccEmailID, subject, body } = approvalEntity[0];
      console.log("listLeaveApprovalEntity", response.data.data.listLeaveApprovalEntity);
  ...
  const handleApprovalSubmit2 = async () => {
    ...
    try {
      ...
      if (statusID === 2) {
        ...
        if (response?.status === 200) {
          const result = response?.data?.data?.listTimesheetApproval?.[0]?.result;
      ...
      // Proceed to email sending if the approval/rejection was successful
      const approvalEntity =
        response?.data?.data?.listTimesheetApproval || response?.data?.data?.listTimesheetApproval;
      ...
      const { fromEmailID, toEmailID, ccEmailID, subject, body } = approvalEntity[0];
      console.log("Email data to be sent:", { fromEmailID, toEmailID, ccEmailID, subject, body });

      if (!fromEmailID || !toEmailID || !subject || !body) {
        console.log("Missing required email fields:", { fromEmailID, toEmailID, subject, body });
  ...
  useEffect(() => {
    ...
    const fetchTimesheetData = async () => {
      try {
        ...
        console.log("Timesheet Data:", response.data.data.listMyApprovalTimesheeEntity);
    ...
    const fetchApprovalPendingData = async () => {
      try {
        ...
        setApprovalPendingData(response.data.data.listMyApprovalPendingCountEntity[0]);
      } catch (error) {
    ...
    const fetchApprovalAgeingData = async () => {
      try {
        ...
        setApprovalAgeingData(response.data.data.listMyApprovalAgeingCountEntity[0]);
      } catch (error) {
  ...
  const currentProjectData = projectData.slice(indexOfFirstItem, indexOfLastItem);
  const currentLeaveData = leaveData.slice(indexOfFirstItem, indexOfLastItem);
  const currentTimesheetData = timesheetData.slice(indexOfFirstItem, indexOfLastItem);
    ...
    <>
      <div className="container">
        ...
        <Tab.Container defaultActiveKey="project" onSelect={handleTabClick}>
          ...
          >
            <Nav.Item>
              <Nav.Link eventKey="project" style={{ color: "black", textDecoration: "none" }}>
                <img src={Project} alt="Project Icon" className="ApprIcns" />
                Project
              </Nav.Link>
            </Nav.Item>
            <Nav.Item>
              <Nav.Link eventKey="leave" style={{ color: "black", textDecoration: "none" }}>
                <img src={Leaves} alt="Leaves Icon" className="ApprIcns" />
                Leaves
              </Nav.Link>
            </Nav.Item>
            <Nav.Item>
              <Nav.Link eventKey="timesheet" style={{ color: "black", textDecoration: "none" }}>
                <img src={ResourceTS} alt="Timesheet Icon" className="ApprIcns" />
                Timesheet
              </Nav.Link>
            </Nav.Item>
          </Nav>
          <div className="row mt-4" style={{ display: "flex" }}>
            <div className="col-lg-4 col-md-4" style={{ width: "30%", paddingRight: "15px" }}>
              <div style={{ width: "90%", height: "300px" }}>
                <h5>Overall Approval Pending</h5>
                <Pie data={pieData} options={options} />
              </div>
              <div className="chart-container mt-3" style={{ width: "80%", height: "300px" }}>
            ...
            <div className="col-lg-8 col-md-8" style={{ width: "70%", paddingLeft: "15px" }}>
                        ...
                        {currentProjectData.length === 0 ? (
                          ...
                          currentProjectData.map((project) => (
                            <tr key={project.projectID}>
                              <td style={{ width: "10%" }}>{project.projectCode}</td>
                              <td style={{ width: "20%" }}>{project.projectName}</td>
                              <td style={{ width: "15%" }}>{project.organizationUnit}</td>
                              <td style={{ width: "15%" }}>{project.expectedStartDate}</td>
                              <td style={{ width: "15%" }}>{project.expectedEndDate}</td>
                              <td style={{ width: "10%" }}>{project.estimatedEfforts}</td>
                              <td style={{ width: "10%" }}>{project.requeststage}</td>
                              <td style={{ width: "15%" }}>{project.revisionDetails}</td>
                        ...
                        {currentLeaveData.length > 0 ? (
                          currentLeaveData.map((leave, index) => (
                            <tr
                              ...
                              onMouseEnter={(e) =>
                                (e.currentTarget.style.backgroundColor = "#c77d7d")
                              }
                              onMouseLeave={(e) =>
                            ...
                            >
                              ...
                              <td>
                                ...
                                >
                                  <span data-bs-toggle="modal" data-bs-target="#approveRevModal">
                                    <Tooltip1 title="Details">
                                      <FontAwesomeIcon
                                ...
                                >
                                  <span data-bs-toggle="modal" data-bs-target="#rejectRevModal">
                                    <Tooltip1 title="Reject">
                                      <FontAwesomeIcon
                                        icon={farThumbsDown}
                                        className="textRed fnt-16"
              ...
              <Box sx={{ display: "flex", justifyContent: "center", marginTop: "20px" }}>
                ...
                <IconButton
                  ...
                  disabled={
                    (activeTab === "project" && currentProjectData.length < itemsPerPage) ||
                    (activeTab === "leave" && currentLeaveData.length < itemsPerPage) ||
                    (activeTab === "timesheet" && currentTimesheetData.length < itemsPerPage)
                  }
                >
                  <ArrowForwardIosIcon />
                </IconButton>
      ...
      >
        ...
        <DialogContent sx={{ minHeight: "100px" }}>
          ...
          <TextField
            label={statusID === 2 ? "Approval Comments:" : "Rejection Comments:"}
            variant="outlined"
            fullWidth
            multiline
            required
      ...
      >
        ...
        <DialogActions>
          <Button onClick={handleApprovalSubmit1} variant="contained" color="success">
            {statusID === 2 ? "Approve" : "Reject"}
          </Button>
          <Button onClick={handleModalClose1} variant="contained" color="error">
            Cancel
          </Button>
        </DialogActions>
      </Dialog>
      ...
      >
        ...
        <DialogActions>
          <Button onClick={handleApprovalSubmit2} variant="contained" color="success">
            {statusID === 2 ? "Approve" : "Reject"}
          </Button>
          <Button onClick={handleModalClose2} variant="contained" color="error">
            Cancel
          </Button>
        </DialogActions>
      </Dialog>

...
const ProjectTaskTable = ({
  ...
  const fetchProjectDetails = async () => {
    try {
      const response = await fetch(
        ...
        }/MyTimesheetEntry/GetTimesheetWeekHeader?InputDate=${
  ...
  const handleSaveEdit = async (taskID, day, newValue) => {
    // e.prevent;
    debugger;
    ...
    //   console.log("check for flag", isFetchedPrevvalue);
    // }

    // setPreviousEfforts((prev) => ({
    //   ...prev,
    //   [`${taskID}-${day}`]:
    //     timesheetData.listProjectDetailsEntity
    //       .flatMap((project) => project.listTaskDetailsEntity)
    //       .find((task) => task.taskID === taskID)?.[`dayEfforst${day}`] || "00:00"
    // }));

    // setPreviousEfforts((prev) => {
    //   if (!prev) prev = {}; // Ensure prev is not undefined/null
    //   return {
    //     ...prev,
    //     [`${taskID}-${day}`]:
    //       timesheetData.listProjectDetailsEntity
    ...
    // var data = { [`${taskID}-${day}`]: foundTask[`dayEfforst${day}`] || "00:00" };

    // setPreviousData(data);

    // console.log("Updated previousEffortsRef in Child:", previousEfforts.current);
  ...
  const handleCheckboxChange = (taskID) => {
    setSelectedTasks((prevState) => {
      ...
      sessionStorage.setItem("selectedTasks", JSON.stringify(updatedSelectedTasks));
    ...
    <tbody>
      <tr className="stickyHeader">
        ...
        {dayNames.map((dayName, dayIndex) => {
          ...
          console.log("topOrangeBrdr", project);
                      ...
                      return (
                        <div key={taskIndex} style={{ marginBottom: "45px" }}>
                          <TextField
                            ...
                            onChange={(e) => handleSaveEdit(task.taskID, day, e.target.value)}
